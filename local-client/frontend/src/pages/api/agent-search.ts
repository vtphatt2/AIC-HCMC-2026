...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET" || req.method === "DELETE") {
    ...
    method = "POST";
    body = JSON.stringify({ query: req.body?.query, top_k: req.body?.top_k });
  } else {
  ...
  try {
    ...
    return res.status(503).json({ error: "Search Agent service is unavailable. Manual search is still available." });
  }
}
