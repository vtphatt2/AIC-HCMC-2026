...
def filter_similar_results(
    ...
    *,
    ...
    """Keep ranking order; remove a result only if every corresponding step
    is similar to one already kept after weighting cosine distance."""
    ...
    for result in results:
        frame_ids = result_frame_ids(result)
        vectors = [normalized.get(frame_id) for frame_id in frame_ids]
