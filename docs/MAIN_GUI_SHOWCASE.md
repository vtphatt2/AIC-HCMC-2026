...
| **VI→EN translation** | Beside the scene-description box | Click once to translate every populated temporal step in one Gemini request. The original Vietnamese text remains visible; each step receives three selectable English video-search paraphrases: direct scene, action/event, and visual context. Search uses the selected paraphrase. |
