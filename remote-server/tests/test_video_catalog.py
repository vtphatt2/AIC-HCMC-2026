...
class VideoCatalogSearchTests(unittest.TestCase):
    ...
    def test_limit_is_applied_after_relevance_ranking(self):
        results = search_video_catalog(VIDEOS, "60 giay", limit=1)

        self.assertEqual([result["video_id"] for result in results], ["L01_V001"])
